...
int handle_math_request(int socket_fd, char *request);
int handle_static_request(int socket_fd, char *request);

#define NONEXISTENT_FILE 1
#define SUCCESS 2
// this returns FAIL (system error - close connection), SUCCESS, or NONEXISTENT_FILE
...
int send_error_response(int socket_fd)
{
  return send_http_response(socket_fd,
                            "Invalid request.\n"
                            "\n"
                            "\"/plus/#/#\" and \"/static/foo\" are the only valid paths.\n");
}

int respond_to_http_request(int socket_fd, char *request)
{
  if ( !strncmp(request,"GET /plus/", 10) )
    return handle_math_request(socket_fd, request);
  if ( !strncmp(request,"GET /static/", 10) )
    return handle_static_request(socket_fd, request);
...
int handle_static_request(int socket_fd, char *request)
{
  char file_path[MAX_MESSAGE_LENGTH];
  ...
  char file_contents[MAX_MESSAGE_LENGTH];
  ...
  {
    return send_http_response(socket_fd,"Nonexistent resource\n");
  }
  return send_http_response(socket_fd, file_contents);
}

// this returns FAIL (system error - close connection), SUCCESS, or NONEXISTENT_FILE
int read_file_contents(const char* file_path, 
  char* buf, 
int buffer_length)
{
